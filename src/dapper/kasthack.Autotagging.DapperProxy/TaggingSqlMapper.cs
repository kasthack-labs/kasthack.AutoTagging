...
public static partial class TaggingSqlMapper
{
     ...
     */
    internal static void DummyCheck()
    {
    ...
    private static Dapper.CommandDefinition ApplyTag(Dapper.CommandDefinition command, string callerMethod, string callerFile, int callerLine)
        => new(ApplyTag(command.CommandText, callerMethod, callerFile, callerLine)!, command.Parameters, command.Transaction, command.CommandTimeout, command.CommandType, command.Flags, command.CancellationToken);
}